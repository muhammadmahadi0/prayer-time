...
function updateCountdown() {
  ...
  const ishraqEnd = sunriseEnd + 120; // 2 hours after sunrise
  const chashtEnd = dhuhr - 10;

  // Check special periods: sunrise prohibited (RED) / Ishraq / Chasht
  const inProhibited = sun > 0 && cm >= sun && cm < sunriseEnd;
  const inIshraq = sun > 0 && cm >= sunriseEnd && cm < ishraqEnd;
  const inChasht = sun > 0 && cm >= ishraqEnd && cm < chashtEnd;
  ...
  if (inIshraq) {
    ...
    $('nextPrayerName').textContent = bn ? 'ইশরাক' : 'Ishraq';
  ...
  if (inChasht) {
    ...
    $('countdownDisplay').textContent = `${pad(ph)}:${pad(pm)}:${pad(ps)}`;
    $('countdownDisplay').className = 'font-mono text-3xl font-bold tracking-[0.06em]';
    $('nextPrayerName').textContent = bn ? 'চাশত' : 'Chasht';
    $('nextPrayerName').className = 'text-lg font-bold mt-0.5';
    $('timeRemainingLabelBn').classList.add('hidden');
    $('timeRemainingLabelEn').classList.add('hidden');
    return;
...
function updateNafil() {
  ...
  const cm = now.getHours() * 60 + now.getMinutes();
  const cs = now.getHours() * 3600 + now.getMinutes() * 60 + now.getSeconds();
  const sunriseEnd = sun + 15; // prohibited period
  const ishraqEnd = sunriseEnd + 120; // 2 hours after sunrise
  const chashtEnd = dhuhr - 10; // ends ~10min before Dhuhr
  ...
  if (sun > 0 && cm >= sunriseEnd && cm < ishraqEnd) {
    ...
    ishraqTimerEl.textContent = '';
  }

  // ---- Chasht timer ----
  const chashtTimerEl = $('nafilChashtTimer');
  if (sun > 0 && cm >= ishraqEnd && cm < chashtEnd) {
    const remainSec = (chashtEnd * 60 - cs + 86400) % 86400;
    const h = Math.floor(remainSec / 3600);
    const m2 = Math.floor((remainSec % 3600) / 60);
    const s = Math.floor(remainSec % 60);
    chashtTimerEl.textContent = bn ? `${pad(h)}:${pad(m2)}:${pad(s)} বাকি` : `${pad(h)}:${pad(m2)}:${pad(s)} left`;
  } else if (sun > 0 && cm < ishraqEnd) {
    // Before Chasht — countdown to Chasht
    const remainSec = (ishraqEnd * 60 - cs + 86400) % 86400;
    const h = Math.floor(remainSec / 3600);
    const m2 = Math.floor((remainSec % 3600) / 60);
    const s = Math.floor(remainSec % 60);
...
function setLang(lang) {
  ...
  $('nafilIshraqTime').textContent = bn ? 'সূর্যোদয় +১৫মি — ২ঘ' : 'Sunrise +15m — 2h';
  $('nafilChasht').textContent = bn ? 'চাশত' : 'Chasht';
  $('nafilChashtTime').textContent = bn ? 'ইশরাক+২ঘ — যোহর' : 'Ishraq+2h — Dhuhr';
